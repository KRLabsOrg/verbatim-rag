...
def get_template_manager(config: Annotated[APIConfig, Depends(get_config)]) -> TemplateManager:
    ...
    if _template_manager is None:
        try:
            ...
            if config.templates_path.exists():
                ...
                logger.info(f"Template manager created and loaded templates from: {config.templates_path}")
            else:
                logger.info(f"Template manager created without templates (file not found: {config.templates_path})")
        except Exception as e:
...
def check_system_ready(
    ...
    try:
        ...
        if not hasattr(rag, 'index') or rag.index is None:
            raise HTTPException(
                status_code=503,
                detail="RAG system is not ready. Index not loaded."
            )
        return True
    except Exception as e:
