...
class LLMClient:
    ...
    def generate_template(
        ...
        try:
            return self.complete(prompt, temperature=0.7)
    ...
    async def generate_template_async(
        ...
        try:
            return await self.complete_async(prompt, temperature=0.7)
        except Exception as e:
...
Return ONLY the template text (no explanation)."""
    ...
    def _fallback_template(self, has_citations: bool) -> str:
        """Generate a simple fallback template."""
        template = """## Response
