...
def load_seed_examples(file_path: str, max_examples: Optional[int] = None) -> List[Dict]:
    ...
    try:
        ...
        for case in archehr_data.cases:
            if not case.clinician_question or not case.sentences or not case.patient_narrative:
                continue
...
def generate_with_llm(prompt: str, model: str = MODEL, max_tokens: int = MAX_TOKENS, temperature: float = TEMPERATURE) -> str:
...
def parse_generated_output(output: str) -> List[Tuple[str, str, str, List[str]]]:
    ...
    for part in parts:
        ...
        if "RELEVANT SENTENCES:" in part:
            ...
            relevant_sentences = [line.strip() for line in rel_text.split("\n") if line.strip()]
...
def generate_examples_task(task_id: int, seed_examples: List[Dict], model: str, examples_per_prompt: int, temperature: float) -> List[Dict]:
...
def main():
    ...
    parser.add_argument("--output_file", type=str, default="data/synthetic/labeled_notes.jsonl", help="Output file path")
    parser.add_argument("--seed_data", type=str, required=True, help="Path to ArchehrData JSON file")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--examples_per_prompt", type=int, default=1, help="Number of examples to generate per prompt")
    parser.add_argument("--temperature", type=float, default=TEMPERATURE, help="Temperature for generation")
    parser.add_argument("--threads", type=int, default=DEFAULT_THREADS, help="Number of threads for parallel processing")
    ...
    num_prompts = (args.num_examples + args.examples_per_prompt - 1) // args.examples_per_prompt
    
    logger.info(f"Generating {args.num_examples} examples using {num_prompts} prompts with {args.threads} threads...")
    ...
    try:
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="Generating examples"):
                ...
                try:
                    ...
                    # Show progress
                    logger.info(f"Task {task_id} completed. Total examples: {len(generated_examples)}")
                    
                    # Save periodically (every 5 tasks or 60 seconds)
                    current_time = time.time()
                    if total_successful % 5 == 0 or (current_time - last_save_time > 60):
                        with open(temp_file, "w") as f:
                            for example in generated_examples:
                                f.write(json.dumps(example) + "\n")
                        last_save_time = current_time
                        logger.info(f"Saved {len(generated_examples)} examples to temporary file")
                ...
                logger.info(f"Progress: {progress:.1f}% ({total_successful} success, {total_failed} failed)")
        ...
        if generated_examples:
            with open(args.output_file, "w") as f:
                for example in generated_examples:
                    f.write(json.dumps(example) + "\n")
            logger.info(f"Saved {len(generated_examples)} examples to {args.output_file}")
    ...
    logger.info(f"Generated {len(generated_examples)} examples. Saved to {args.output_file}")
